...
class ChatResponse(BaseModel):
    ...
    sources: List[str] = Field(
        ..., description="List of document sources used for the answer"
    )
...
async def api_root():
    ...
    return {
        ...
        "authentication": (
            "API key required for all endpoints (except docs and health)"
...
async def validate_api_key(api_key: str = Depends(get_api_key)):
    ...
    """
    return APIKeyResponse(success=True, message="API key is valid")


@app.get(
    "/api/generate-key", response_model=dict, include_in_schema=False
)  # Not in public docs for security
...
async def http_exception_handler(request, exc):
    return JSONResponse(
        ...
        content=ErrorResponse(
            success=False, error=exc.detail, detail=getattr(exc, "detail", None)
        ).dict(),
    )
...
async def general_exception_handler(request, exc):
    ...
    return JSONResponse(
        status_code=500,
        content=ErrorResponse(
            success=False, error="Internal server error", detail=str(exc)
        ).dict(),
    )
